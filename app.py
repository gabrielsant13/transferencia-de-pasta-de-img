...
while sysdate().split()[2] < '2027': # Trava de tempo
    ...
    for cliente in clientes:
        try:
            ...
            # Salva no log
            arquivo = open(caminho+'log.txt',"a")
            arquivo.write(f'Cliente não cadastrado ou não existe: {cliente} | {e} - {datahora()}\n')
            arquivo.close()

            print(f'Cliente não cadastrado ou não existe: {cliente} | {e} - {datahora()}')
    ...
    # Execução do processo a cada 1h, a partir da inicialização do serviço
    time.sleep(3600)
